    ...
    if (bytes_read > 0) {
        if (cmd.type == CMD_READ) {
            ...
            if (!fp) {
                ...
                if (!parsed) {
                    free(file_content);
                    send(client_socket, "ERROR: Out of memory on SS\n", 27, 0);
                ...
                if (sentence_index < 0 || sentence_index > parsed->sentence_count) {
                    ...
                    if (sentence_index == parsed->sentence_count && sentence_index < 100) {
                        parsed->sentence_count++;
                        parsed->sentences[sentence_index].word_count = 0;
                        parsed->sentences[sentence_index].delimiter = '.';
                    }
